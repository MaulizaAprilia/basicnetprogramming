...
const geocodeURL = 'https://api.mapbox.com/geocoding/v5/mapbox.places/Universitas%20Negeri%20Padang.json?access_token=pk.eyJ1IjoibWF1bGl6YWFwcmxhIiwiYSI6ImNtMWtoZWVvNjA5eDYyaW9wdWx5NXNkOXEifQ.W8RgS540BQZg_zeYw20LxQ&limit=5';
...
request({ url: geocodeURL, json: true }, (error, response) => {
    ...
    // Filter results for Indonesia (country code is 'ID')
    const validLocations = locations.filter(location => 
        location.context.some(context => context.short_code === 'id')
    );

    if (validLocations.length === 0) {
        return console.log('No valid locations found in Indonesia.');
    }

    
    // First location
    const latitude1 = validLocations[0].center[1];
    const longitude1 = validLocations[0].center[0];
    ...
    // If there is a second valid location
    if (validLocations.length > 1) {
        const latitude2 = validLocations[1].center[1];
        const longitude2 = validLocations[1].center[0];
        ...
        console.log('No second location found in Indonesia.');
    }
});
